  ...
  return (
    <main>
          <header className="border-three-500 border-b-4">
            <nav className={`${open ? "flex flex-col" : "flex flex-row"} justify-between py-4 px-8 text-xl md:py-2 md:px-14`}>
              <div className="flex justify-between w-full items-center">
                <h2 className="text-three-500 font-bold text-2xl">Coobro</h2>
                <FaAlignCenter className="block text-2xl md:hidden" onClick={() => setOpen(!open)}/>
              </div>
              <ul className={`${open ? "block" : "hidden"} font-semibold md:flex md:justify-center md:items-center`}>
                <div className="flex flex-col justify-between items-center gap-4 mt-8 text-base md:flex md:flex-row md:mt-0 md:text-xl">
